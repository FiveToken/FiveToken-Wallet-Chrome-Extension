...
import networkForm from '@/pages/setting-networks/component/networks-form.vue'
...
describe('set-netowrk index.vue', () => {
  ...
  const wrapper = shallowMount(networkForm, {
    store,
    localVue,
    data () {
      return {
  ...
  it('networkForm .vue-test', async () => {
    wrapper.vm.closeDelete()
    wrapper.vm.back()
    wrapper.vm.deleteNetwork()
