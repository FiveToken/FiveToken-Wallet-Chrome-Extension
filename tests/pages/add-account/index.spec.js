...
import addAccount from '@/pages/add-account/component/index.vue'
...
describe('account index.vue', () => {
  ...
  const wrapper = shallowMount(addAccount, {
    store,
    localVue,
    data () {
      return {
  ...
  it('add-account .vue-test', async () => {
    await wrapper.vm.layoutMounted()
    wrapper.vm.nameChange('account')
    wrapper.vm.confirm()
    wrapper.vm.closePassword()
    wrapper.vm.confirmPassword('Aa123456789012zpc')

    wrapper.vm.back()
    const href5 = window.location.href
    expect(href5.indexOf('account')).not.toBe(-1)
  })
})
