...
describe('fivetoken-connect index.vue', () => {
  let localStore
  ...
  const wrapper = shallowMount(fivetokenConnect, {
    store,
    localVue,
    data () {
      return {
  ...
  it('fivetoken-connect .vue-test', async () => {
    await wrapper.vm.addAccount()
    const href = window.location.href
    expect(href.indexOf('add-account')).not.toBe(-1)
  })
})
